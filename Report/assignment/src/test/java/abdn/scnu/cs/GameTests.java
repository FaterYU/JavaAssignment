...
public class GameTests {
	...
	public void t1_1_a() {
		...
		try {
			...
			fail(ANSI_WHITE_BACKGROUND + ANSI_BLACK
					+ "T1.1_a: Make sure the constructor of BattleShip class is modified and the ship name is stored in appropriate variable"
					+ ANSI_RESET + e);
		}
	...
	public void t1_1_b() {
		...
		if (horizontal < 20 || vertical < 20) {
			fail(ANSI_WHITE_BACKGROUND + ANSI_BLACK
					+ "T1.1_b The ship orientation should be decided at random. Out of 100 ships you have " + vertical
					+ " positioned vertically and " + horizontal
					+ " positioned horizontally. Thisi does not look random." + ANSI_RESET);
	...
	public void t1_2() {
		try {
			...
			fail(ANSI_WHITE_BACKGROUND + ANSI_BLACK
					+ "T1.2 It seems the getter and setter methods are not working as expected. " + ANSI_RESET + e);
		}
	...
	public void t1_3() {
		try {
			...
			fail(ANSI_WHITE_BACKGROUND + ANSI_BLACK
					+ "T1.3 It seems that the checkAttack method is not working as expected. " + ANSI_RESET + e);
		}
	...
	public void t2_1() {
		...
		try {
			...
			fail(ANSI_WHITE_BACKGROUND + ANSI_BLACK
					+ "T2.1: Are you initialising the GameGrid with . ? Does it extend teh AbstractGameGrid?   "
					+ ANSI_RESET + e);
		}
	...
	public void t2_2() {
		...
		try {
			...
			fail(ANSI_WHITE_BACKGROUND + ANSI_BLACK
					+ "T2.2: Are you generating the ships based on the parameter passed to the constructor of GameGrid? Do you generate the names of ships as specifiied?   "
					+ ANSI_RESET + e);
		}
	...
	public void t2_3() {
		...
		try {
			...
			if (shipLength < 6) {
				fail(ANSI_WHITE_BACKGROUND + ANSI_BLACK + "T2.3: Are you generating required number of ships?   "
						+ ANSI_RESET);
			...
			// allow for some overlap
			if (gameGrid3.ships[0].shipCoordinates[0][0] == gameGrid.ships[0].shipCoordinates[0][0]
					&& gameGrid3.ships[0].shipCoordinates[0][1] == gameGrid.ships[0].shipCoordinates[0][1]) {
				fail(ANSI_WHITE_BACKGROUND + ANSI_BLACK + "T2.3: Are you possitioning your ships randomly?   "
						+ ANSI_RESET);
			...
			fail(ANSI_WHITE_BACKGROUND + ANSI_BLACK
					+ "T2.3: Something is wrong with your method of placing ships on the grid.   " + ANSI_RESET + e);
		}
	...
	public void t2_4() {
		...
		try {
			...
			fail(ANSI_WHITE_BACKGROUND + ANSI_BLACK
					+ "T2.4: Something is wrong with your methods. Are you hiding opponnet's ships? Do you extend the GameGrid class?    "
					+ ANSI_RESET + e);
		}
	...
	public void t3_1() {
		...
		try {
			...
			fail(ANSI_WHITE_BACKGROUND + ANSI_BLACK
					+ "T3.1: Something is wrong. Are you intantiating the grids? Do you extend the GameControls interface?    "
					+ ANSI_RESET + e);
		}
	...
	public void t3_3() {
		...
		try {
			...
			fail(ANSI_WHITE_BACKGROUND + ANSI_BLACK
					+ "T3.3: Something is wrong. Is your check victory method working? Check the code of this test for hints on the expected behaviour.    "
					+ ANSI_RESET + e);
		}
	...
	public void t3_4() {
		...
		try {
			...
			// we only have one ship and it can't have this shape so there has to be at
			// least one miss
			game.playRound("0,0");
			game.playRound("0,1");
			outContent.reset();
			...
			fail(ANSI_WHITE_BACKGROUND + ANSI_BLACK
					+ "T3.4: Something is wrong with you rplayRound method. Check your code and also code of this test to see what is expected when your method is called.    "
					+ ANSI_RESET + e);
		}
